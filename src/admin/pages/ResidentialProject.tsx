...
import ResidentialHeroSection from "../components/residential/ResidentialHeroSection";
...
import ResidentialAmenitiesSection from "../components/residential/ResidentialAmenitiesSection";
import LocationAdvantageSection from "../components/residential/LocationAdvantageSection";
import UnitLayoutSection from "../components/residential/UnitLayoutSection";
import ResidentialGallerySection from "../components/residential/ResidentialGallerySection";
import ResidentialFaqSection from "../components/residential/ResidentialFaqSection";

import {
  type ProjectMeta,
  type ResidentialHeroSectionData,
  type ResidentialOverviewSectionData,
  type ResidentialHighlightsSectionData,
  type ImmersiveTourSectionData,
  type ResidentialAmenitiesSectionData,
  type ResidentialLocationSectionData,
  type UnitLayoutSectionData,
  type ResidentialGallerySectionData,
  type ResidentialFaqSectionData,
...
export default function ResidentialProject() {
  ...
  const [hero, setHero] = useState<ResidentialHeroSectionData>({
    logo: "",
    background_banner: ""
  });

  const [overview, setOverview] = useState<ResidentialOverviewSectionData>({
    heading: "",
    description: "",
    key_features: [],
    image: ""
  ...
  const [highlights, setHighlights] = useState<ResidentialHighlightsSectionData>({
    items: []
  });
  ...
  const [amenities, setAmenities] = useState<ResidentialAmenitiesSectionData>({
    items: []
  });

  const [location, setLocation] = useState<ResidentialLocationSectionData>({
    locations: [],
    map_location: ""
  });
  ...
  const [gallery, setGallery] = useState<ResidentialGallerySectionData>({
    images: [],
    youtube_links: []
  });

  const [faqs, setFaqs] = useState<ResidentialFaqSectionData>({
    items: []
  });
  ...
  const saveToDatabase = async () => {
    try {
      ...
      if (
        ...
        overview.image ||
        overview.key_features.length > 0
      ) {
        await addSection({
          project_id: id!,
      ...
      if (highlights.items.length > 0) {
        await addSection({
          project_id: id!,
          section_type: "project_highlights",
          position: 3,
          data: highlights
        });
      }
      ...
      // IMMERSIVE TOUR
      if (immersiveTour.url1 || immersiveTour.url || immersiveTour.image) {
        await addSection({
          project_id: id!,
          section_type: "immersive_tour",
          position: 4,
          data: immersiveTour
        });
      }
      ...
      // LOCATION ADVANTAGE
      if (location.locations.length > 0 || location.map_location) {
        await addSection({
          project_id: id!,
          section_type: "location_advantage",
          position: 6,
          data: location
        });
      }
      ...
      // GALLERY
      if (gallery.images.length > 0 || gallery.youtube_links.length > 0) {
        await addSection({
          project_id: id!,
      ...
      alert("Residential project saved successfully ✅");
    } catch (err) {
      console.error(err);
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-600 mt-2 text-lg ml-4">
                Create and manage residential real estate projects. Fill in the details below to publish a new project.
              </p>
            </div>
  ...
  {project.project_thumbnail && (
    <div style={{ marginTop: 8 }}>
      ...
      />
            ...
            {/* HERO */}
            <ResidentialHeroSection data={hero} onChange={setHero} />

            {/* OVERVIEW */}
            <ResidentialOverviewSection data={overview} onChange={setOverview} />
