...
const Prodamenities = ({ amenitiesSection, projectTitle = "Sapphire57" }) => {
  // --- ADJUSTABLE SETTINGS ---
  const settings = {
  ...
  const amenities = amenitiesSection && amenitiesSection.items && Array.isArray(amenitiesSection.items)
    ? amenitiesSection.items.map(item => ({
        title: item.name || item.title || "Amenity",
        img: item.image || defaultAmenities[0].img
      }))
    : defaultAmenities;
      ...
      <div className="max-w-[1450px] mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <motion.h2 
            ...
            className={`xs:text-[22px] text-[28px] md:text-[34px] lg:text-[32px] xl:text-[48px] ${settings.titleFont} text-black tracking-tight`}
          >
            {projectTitle} Amenities
          </motion.h2>
        </div>
